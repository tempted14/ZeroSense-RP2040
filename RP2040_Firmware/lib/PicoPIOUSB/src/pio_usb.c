...
void __not_in_flash_func(pio_usb_bus_usb_transfer)(pio_port_t *pp,
  ...
  // The EOP program-counter window lasts only a few bit times. Polling that
  // address can miss it and spin forever while the other RP2350 core remains
  // responsive (Pico-PIO-USB issue #197, PR #206 commit 02ca10b). Wait the
  // fixed EOP tail instead. Include the fractional divider: at 120 MHz the
  // full-speed TX state machine uses 2.5 CPU cycles per SM cycle.
