...
**Arm output** does not continuously move the pointer. RP2040 activation uses the desktop's foreground-gated, per-device Raw Input M1+M2 monitor plus a 750 ms keepalive. This keeps RP2040-generated M1 releases from stopping a held physical trigger. The RP2040 is still a separate mouse: its rapid-fire pulses may not register as new shots while physical M1 stays down, depending on the game's handling of multiple mice. RP2350 activation is different by design: the app grants a short foreground/armed lease, while the board reads raw M1+M2 from the downstream mouse itself. A raw release or mouse/interface disconnect stops output immediately; synthesized rapid-fire reports cannot feed back into this trigger. Physical movement, buttons, wheel, and pan remain additive and live.
