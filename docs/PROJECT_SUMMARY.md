...
| GENERAL_SETTINGS | `FD` | Default-off timing variance and balanced generated-delta noise toggles |
| RESET | `FF` | Empty; enter UF2 mode |

The current app sends version 2 profiles inside an all-or-nothing v4 configuration transaction. Configuration schema 3 adds the persisted, default-off General-mode ±8% timing-variance setting and balanced ±2–3-count generated-delta noise to exact readback and the canonical hash. Pattern and rapid-fire timing remain exact, physical RP2350 deltas are never randomized, and every injected noise offset is repaid before another is introduced. The firmware snapshots the last-good state and commits only after exact readbacks, a complete pattern, and an identical canonical FNV-1a hash. Frames use a sync word, sequence, bounded one-byte length, CRC-16, and partial-frame timeout recovery. Pattern points remain bounded chunks. General mode schedules movement every 8 ms; pattern mode uses phase-locked `60,000,000 / RPM` deadlines and distributes each point across 1 ms frames with Q16 remainder preservation. Semi-automatic values are explicitly converted from counts per reference frame to displacement per shot. The shared scheduler is fake-clock testable, retains absolute deadlines across stalls, and exposes delayed-frame, maximum-lateness, live-queue, report-interval, and General-`dt`-clamp telemetry. Experimental calibration remains host-side.
